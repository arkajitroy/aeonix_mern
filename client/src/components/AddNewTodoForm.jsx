...
const AddNewTodoForm = () => {
  ...
  return (
    <div className="bg-violet-50 w-[90%] md:w-[80%] lg:w-[50%] p-5 rounded-md shadow-md">
      ...
      <form
        ...
        <input type="text" className="my-2 w-full border-2 border-0 px-5 py-2 rounded-sm my-2" placeholder="Add here todo title" value={todoTitle} onChange={(e) => setTodoTitle(e.target.value)} />

        {/* TODOS DESCRIPTION INPUT FIELD */}
        <textarea className="my-2 w-full border-2 border-0 px-5 py-2 rounded-sm" cols="30" rows="3" placeholder="Add here todo description" value={todoDescription} onChange={(e) => setTodoDescription(e.target.value)} />

        {/* TODOS ADD BUTTON */}
        <button type="submit" className="bg-indigo-500 text-white rounded-md shadow-md w-full my-2 py-2 transition-all duration-400 hover:bg-indigo-800">
